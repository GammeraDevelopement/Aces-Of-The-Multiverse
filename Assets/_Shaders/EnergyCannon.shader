Shader "Gammera/Particles/EnergyCannon"
{
	Properties{
			...
			SubShader {
				Pass {
					...
					v2f vert(appdata_t v)
					{
						...
						#endif
						float rim = pow(saturate(dot(ObjSpaceViewDir(v.vertex),v.normal)),5);
						o.color = v.color * rim;
						o.custom1 = float4(v.texcoord.zw, v.texcoord2.xy);
					...
					fixed4 frag(v2f i) : SV_Target
					{
						...
						float noise = saturate(pow(tex2D(_Noise, float2(i.texcoord.x + (_Time.y * i.custom1.x), i.texcoord.y + (_Time.y * i.custom1.y))).x * i.custom1.z, i.custom1.w));
